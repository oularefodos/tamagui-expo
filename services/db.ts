...
const localDb = {
    get: async (key: string) => {
        try {
            ...
            console.error(`[localDb] Read failed for '${key}'`, e);
            return [];
        }
    },
    set: async (key: string, value: any) => {
        try {
            ...
            console.error(`[localDb] Save failed for '${key}'`, e);
        }
    },
};
...
export const db = {
    from: (table: string) => ({
        ...
        insert: async (payload: any) => {
            ...
            console.log(`[MOCK] Saved to '${table}'`, newItem);
        ...
        update: (payload: any) => {
            ...
            console.log(`[MOCK] Update simulated for '${table}'`);
            return {
                eq: async (col: string, val: any) => {
                    try {
                        const current = await localDb.get(table);
                        const updated = current.map((item: any) =>
                            item[col] === val ? { ...item, ...payload } : item
                        );
                        ...
                        console.log(
                            `[MOCK] Updated ${updatedItems.length} item(s) in '${table}'`
                        );
                        return { data: updatedItems, error: null };
                    } catch (e) {
                        console.error(`[MOCK] Update failed for '${table}'`, e);
        ...
        delete: () => {
            ...
            console.log(`[MOCK] Delete simulated for '${table}'`);
            return {
                eq: async (col: string, val: any) => {
                    try {
                        const current = await localDb.get(table);
                        ...
                        console.log(
                            `[MOCK] Deleted ${toDelete.length} item(s) from '${table}'`
                        );
                        return { data: toDelete, error: null };
                    } catch (e) {
                        console.error(`[MOCK] Delete failed for '${table}'`, e);
                        return { data: null, error: e };
                    }
                },
            };
